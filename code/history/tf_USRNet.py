...
from torchvision.models.resnet import BasicBlock as B
import numpy as np
...
def splits(a, sf):
    ...
    '''
    b = tf.stack(tf.split(a, sf, axis=2), axis=5)
    b = tf.concat(tf.split(b, sf, axis=3), axis=5)
    return b
...
def crdiv(x, y):
    ...
    a, b = x[..., 0], x[..., 1]
    return tf.stack([a/y, b/y], -1)

## done
def csum(x, y):
    # complex + real
    return tf.stack([x[..., 0] + y, x[..., 1]], -1)

## done
def cabs(x):
    # modulus of a complex number
    a = x[..., 0]**2+x[..., 1]**2
    b = np.full(a.shape, 0.5)
    return tf.math.pow(x[..., 0]**2+x[..., 1]**2, 0.5)
...
def cmul(t1, t2):
    ...
    return tf.stack([real1 * real2 - imag1 * imag2, real1 * imag2 + imag1 * real2], axis=-1)
...
def rfft(t):
    ...
    return tf.signal.rfft2d(t)

## done
def irfft(t):
    # Complex-to-real Inverse Discrete Fourier Transform
    return tf.signal.irfft2d(t)

## done
def fft(t):
    # Complex-to-complex Discrete Fourier Transform
    return tf.signal.fft2d(t)

## done
def ifft(t):
    # Complex-to-complex Inverse Discrete Fourier Transform
    return tf.signal.ifft2d(t)
...
def p2o(psf, shape):
    ...
    Returns:
        otf: NxCxHxWx2
    '''
    otf = tf.zeros(psf.shape[:-2] + shape, dtype=psf.dtype)
    otf[...,:psf.shape[2],:psf.shape[3]].identity(psf)
    for axis, axis_size in enumerate(psf.shape[2:]):
        otf = tf.roll(otf, -int(axis_size / 2), axis=axis+2)
    otf = rfft(otf)
    a = tf.experimental.numpy.log2(tf.cast(psf.shape, psf.dtype))
    n_ops = tf.math.reduce_sum(tf.cast(psf.shape, psf.dtype) * a)
    otf[..., 1][tf.math.abs(otf[..., 1]) < n_ops*2.22e-16] = tf.cast(tf.constant(0), psf.dtype)
    return otf
...
def upsample(x, sf=3):
    ...
    '''
    st = 0
    z = tf.cast(tf.zeros([x.shape[0], x.shape[1], x.shape[2]*sf, x.shape[3]*sf]), x.dtype)
    z[..., st::sf, st::sf].identity(x)
    return z
...
class ResUNet():
    def __init__(self, in_nc=4, out_nc=3, nc=[64, 128, 256, 512], nb=2, act_mode='R', downsample_mode='strideconv', upsample_mode='convtranspose'):
        ...
        self.m_head = BasicBlock.conv(in_nc, nc[0], bias=False, mode='C')
